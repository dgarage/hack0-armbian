...
if $BUILD; then
    ...
    cd "$OVERLAY_DIRECTORY"
    if ! [ -f "utxo-snapshot-bitcoin-mainnet-744358.tar" ]; then
        set +e
        rm utxo-snapshot-*.tar &> /dev/null
        set -e
        wget "http://utxosets.blob.core.windows.net/public/utxo-snapshot-bitcoin-mainnet-744358.tar" -q --show-progress
    fi
    popd
